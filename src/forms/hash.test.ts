...
describe('canonicalJSONHash', () => {
  ...
  it('changes when any leaf value differs', async () => {
    ...
    const [h1, h2] = await Promise.all([canonicalJSONHash(base), canonicalJSONHash(mutated)]);
    ...
    const [hR, hS] = await Promise.all([canonicalJSONHash(reordered), canonicalJSONHash(swapped)]);
  ...
  it('produces a stable, deterministic hash for trivial inputs (e.g. {})', async () => {
    ...
    expect(h1).toBe('44136fa355b3678a1146ad16f7e8649e94fb4fc21fe77e8310c060f61caaff8a');
